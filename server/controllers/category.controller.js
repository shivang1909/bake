...
export const AddCategoryController = async (request, response) => {
  try {
    ...
    const originalPath = request.file.path; // e.g. 'uploads/image-1234.png'
    const webpFileName = path.parse(request.file.filename).name + ".webp";
    const webpPath = path.join("uploads", webpFileName); // 'uploads/image-1234.webp'
    ...
    // Delete original uploaded file (png/jpg)
    fs.unlinkSync(originalPath);

    // Save webp image path in DB
    const addCategory = new CategoryModel({
      name,
      image: webpPath, // Save the new webp path
    });
