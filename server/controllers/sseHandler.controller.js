...
const orderstatuschange = (data) => {
  console.log("Notification check data order status change:", data); // Debugging line
...
const deliveryPartnerNotification = (data) => {
  console.log("Notification check data delivery partner notification:", data);

  const isArray = Array.isArray(data.deliveryPartnerId);

  for (const [id, client] of clients.entries()) {
    if (client.role === "Delivery Partner") {
      const shouldNotify = isArray
        ? data.deliveryPartnerId.includes(id)
        : id === data.deliveryPartnerId;

      if (shouldNotify) {
        console.log(`📬 Sending notification to Delivery Partner ID: ${id}`);
        client.res.write(
