...
const ProductForm = ({ close, isEdit = false, updatedata }) => {
  ...
  const [data, setData] = useState({
    ...
    weightVariants: isEdit
      ? updatedata.weightVariants
      : [{ weight: "", price: "", qty: "", discount: "" }],
    sku_code: isEdit ? updatedata.sku_code : "",
    shelf_life: isEdit ? updatedata.shelf_life : "",
    checkcategory: false,
  });
...
const handleWeightVariantChange = (index, selectedValue, name) => {
  ...
  if (
    ...
    updatedVariant[name] = selectedValue.value;
  } else {
  ...
  const addUpload = (e) => {
    ...
    const hasDuplicate = allselectedfiles.some((newFile) => {
      const isDuplicate = data.image.some(
  ...
  const handleEditUpload = async (e) => {
    ...
    const hasDuplicate = allSelectedFiles.some((newFile) => {
      if (
        imageNames.includes(newFile.name) ||
        newImageNames.includes(newFile.name)
      ) {
          ...
          >
            ...
            <div>
              {data.weightVariants.map((variant, index) => (
                <div key={index} className="flex gap-4 mb-2">
                  ...
                  <div className="flex-1">
                    <WeightVariantDropdown
                      value={variant.weight} // Just pass the string value
                      onChange={handleWeightVariantChange}
                      index={index}
                    />
                  </div>
                  ...
                  <div className="flex-1">
                    <input
                      ...
                      value={variant.qty}
                      onChange={(e) => {
                        handleWeightVariantChange(index, e.target.value, "qty");
                      }}
                      className="bg-blue-50 p-2 outline-none border focus-within:border-primary-200 rounded w-full"
                    />
                  </div>
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      const updatedVariants = data.weightVariants.filter(
                        (_, i) => i !== index
                      );
                      setData((prev) => ({
                        ...prev,
              ...
              <button
                ...
                className="bg-primary-100 hover:bg-primary-200 py-1 rounded"
              >
                Add Weight Variant
              </button>
            </div>
