    ...
    const updateFilterOnce = () => {
      ...
      if (oldShelfLife) {
        if (oldShelfLife !== maxshelfLife) {
          setFilter((prev) => {
            const withoutOldShelfLife = prev.filter(
              (item) => item !== oldShelfLife
            );
            if (maxshelfLife > 0)
              return [...withoutOldShelfLife, `${maxshelfLife} Day`];
            // Add new shelf life
            else return withoutOldShelfLife;
          });
          return;
        }
      ...
      if (oldPriceRange) {
        ...
        if (priceRange[0] !== num1 || priceRange[1] !== num2) {
          setFilter((prev) => {
            const withoutOldPriceRange = prev.filter(
              (item) => item !== oldPriceRange
            );
            if (priceRange[0] !== 10 || priceRange[1] !== 1000)
              return [
                ...withoutOldPriceRange,
                `${priceRange[0]}₹-${priceRange[1]}₹`,
              ];
            else return withoutOldPriceRange;
          });
          return;
        }
